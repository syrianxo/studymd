...
// Shows: color picker, study mode buttons, topics, scrollable slide deck
// with lightbox, slide→flashcard linking, and manage actions (hide/archive).
'use client';
...
// ─── Slide → Flashcard Linking ───────────────────────────────────────────────

function getFlashcardsForSlide(flashcards: Flashcard[], slideNumber: number): Flashcard[] {
  return flashcards.filter(fc => fc.slide_number === slideNumber);
}
...
export default function LectureViewModal({
  ...
  const { urls: slideUrls, state: slideState } = useSlides(
    lecture.internal_id,
  ...
  // Flashcards linked to the currently selected slide
  const linkedCards = selectedSlide !== null
    ? getFlashcardsForSlide(flashcards, selectedSlide)
    : [];
  ...
  return createPortal(
    <div
      ref={overlayRef}
      className="lvm-overlay"
      onClick={(e) => { if (e.target === overlayRef.current) onClose(); }}
    >
      <style>{modalCss}</style>

      <div className="lvm-modal" role="dialog" aria-modal="true" aria-label={title}>
        {/* Drag handle (mobile) */}
        <div className="lvm-drag-handle" />

        {/* ── Header: Color picker + Title + Close ── */}
        <div className="lvm-header">
          <div className="lvm-color-row">
            {PRESET_COLORS.map(c => (
              <button
                key={c}
          ...
          <button className="lvm-close" onClick={onClose} aria-label="Close">✕</button>
        </div>
        ...
        <div className="lvm-info">
          ...
          <div className="lvm-info-text">
            <h2 className="lvm-title">{title}</h2>
            {lecture.subtitle && <p className="lvm-subtitle">{lecture.subtitle}</p>}
            <span className="lvm-course" style={{ background: `${localColor}22`, color: localColor }}>
              {course}
            </span>
          </div>
        </div>
        ...
        <div className="lvm-study-btns">
          <button className="lvm-study-btn flash" onClick={onFlashcards}>
            <span className="lvm-btn-icon">📇</span>
            <span className="lvm-btn-label">Flashcards</span>
            <span className="lvm-btn-sub">Review &amp; memorize</span>
          </button>
          <button className="lvm-study-btn exam" onClick={onExam}>
            <span className="lvm-btn-icon">📝</span>
            <span className="lvm-btn-label">Practice Exam</span>
            <span className="lvm-btn-sub">Test your knowledge</span>
          </button>
        </div>
        ...
        {slideState === 'loading' && (
          <div className="lvm-slide-loading">
            <span className="lvm-spinner">⟳</span> Loading slides…
          </div>
        )}
        {slideState === 'empty' && (
          ...
          <div className="lvm-slide-strip">
            {slideUrls.map((url, i) => (
              <div
                ...
                onClick={() => {
                  setSelectedSlide(prev => prev === i + 1 ? null : i + 1);
                }}
                role="button"
                tabIndex={0}
              ...
              >
                ...
                <button
                  className="lvm-slide-expand"
                  onClick={(e) => { e.stopPropagation(); setLightboxIdx(i); }}
                  aria-label={`View slide ${i + 1} full screen`}
                >⛶</button>
              </div>
            ))}
          </div>
        ...
        {/* ── Linked Flashcards (when a slide is selected) ── */}
        {selectedSlide !== null && (
        ...
        {(onHide || onArchive) && (
          <>
            ...
            <div className="lvm-manage-actions">
              {onHide && (
                <button className="lvm-manage-btn" onClick={() => { onHide(); onClose(); }}>
                  <span>👁</span> Hide Lecture
                </button>
              )}
              {onArchive && (
                <button className="lvm-manage-btn danger" onClick={() => { onArchive(); onClose(); }}>
                  <span>📦</span> Archive Lecture
                </button>
              )}
            </div>
          </>
...
.lvm-overlay {
  ...
  padding: 0;
  animation: lvm-fade-in 0.2s ease;
}
...
.lvm-modal {
  ...
  animation: lvm-slide-up 0.35s cubic-bezier(0.34, 1.2, 0.64, 1) forwards;
  scrollbar-width: thin;
  scrollbar-color: rgba(255,255,255,0.1) transparent;
}
@keyframes lvm-slide-up {
...
.lvm-drag-handle {
  ...
  background: var(--border-bright, rgba(255,255,255,0.15));
  border-radius: 2px;
  margin: 0 auto 14px;
}
...
.lvm-close {
  ...
  color: var(--text-muted, #6b7280); font-size: 16px;
...
.lvm-subtitle {
  font-size: 13px; color: var(--text-muted); line-height: 1.5; margin-bottom: 6px;
}
.lvm-course {
...
.lvm-study-btn {
  ...
  gap: 5px; padding: 14px 12px; border-radius: 10px; cursor: pointer;
  border: 1px solid rgba(255,255,255,0.1);
  background: var(--surface2, #1a1e27);
  font-family: 'Outfit', sans-serif; color: var(--text);
  transition: all 0.15s; min-height: 44px;
}
.lvm-study-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(0,0,0,0.2); }
...
.lvm-btn-icon { font-size: 20px; line-height: 1; }
...
.lvm-spinner {
  ...
  animation: lvm-spin 1s linear infinite;
}
...
.lvm-linked-q {
  ...
  font-weight: 600; color: var(--text); margin-bottom: 6px; line-height: 1.3;
}
.lvm-linked-a {
...
.lvm-linked-topic {
  ...
  background: rgba(91,141,238,0.1); padding: 2px 8px; border-radius: 50px;
...
/* Manage actions */
.lvm-divider {
  height: 1px; background: var(--border); margin: 20px 0 14px;
}
.lvm-manage-actions {
...
.lvm-manage-btn:hover {
  border-color: var(--border-bright); color: var(--text);
  background: rgba(255,255,255,0.04);
}
.lvm-manage-btn.danger { color: var(--danger, #ef4444); }
.lvm-manage-btn.danger:hover {
  border-color: rgba(239,68,68,0.3); background: rgba(239,68,68,0.08);
}
...
@media (min-width: 768px) {
  ...
  .lvm-color-dot { width: 22px; height: 22px; }
}

/* Mobile touch targets */
@media (max-width: 639px) {
  .lvm-color-dot { width: 28px; height: 28px; }
  .lvm-color-row { gap: 8px; }
  .lvm-study-btn { padding: 12px 8px; }
