...
function SectionDivider() {
  return <div className="prf-divider" />;
}
...
export default function ProfilePage() {
  ...
  // UI state
  const [saving, setSaving]               = useState<string | null>(null); // which field
  ...
  async function saveProfile(field: 'displayName' | 'username') {
    ...
    try {
      ...
      if (field === 'displayName') setDisplayName(data.profile.displayName ?? '');
      if (field === 'username')    setUsername(data.profile.username ?? '');
    } finally {
      setSaving(null);
    }
  }
  ...
  async function saveEmail() {
    if (!newEmail || newEmail === email) { setMsg('email', false, 'No change.'); return; }
    setSaving('email');
    try {
  ...
  async function deleteAccount() {
    ...
    try {
      const supabase = createClient();
      // Sign out first, then let Supabase cascade delete handle the rest
      // (or call a server action that uses service role to delete the user)
  ...
  function Msg({ k }: { k: string }) {
    ...
    if (!m) return null;
    return (
      <span className={`prf-feedback ${m.ok ? 'ok' : 'err'}`}>{m.msg}</span>
    );
  }
        ...
        <section className="prf-section prf-avatar-section">
          ...
          <div className="prf-avatar-meta">
            ...
            <div className="prf-avatar-sub">
              <span className={`prf-role-badge prf-role-${profile?.role}`}>
                {profile?.role}
              </span>
              {profile?.isPrimary && (
                <span className="prf-primary-badge">⭐ primary</span>
              )}
            </div>
          </div>
        </section>
          ...
          <div className="prf-field-group">
            ...
            <div className="prf-inline-row">
              <div className="prf-input-prefix-wrap">
                ...
                <input
                  id="username"
                  className="prf-input prf-input-with-prefix"
                  type="text"
                  value={username}
          ...
          <div className="prf-stats-grid">
            ...
            <div className="prf-stat-card">
              <span className="prf-stat-val accent">
                {stats?.avgScore !== null && stats?.avgScore !== undefined ? `${stats.avgScore}%` : '—'}
              </span>
...
/* Inline row: input + save button side by side */
.prf-inline-row {
  display: flex;
  gap: 8px;
  align-items: stretch;
...
@media (max-width: 767px) {
  ...
  .prf-modal-cancel, .prf-danger-btn { width: 100%; justify-content: center; }
}
