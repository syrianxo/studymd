...
- [x] 🔴 Enable RLS + policies on `subscription_tiers` — *ADR-016*
- [x] 🔴 Replace hard-coded admin UUID in `api_usage` policy — *ADR-017*
- [x] 🔴 Tighten `slides` bucket SELECT policy — *ADR-018 (Option A)*
- [x] 🔴 Add policy for `system_config` — *ADR-019 (intentional no-user-policy)*
- [x] 🔴 Set `search_path` on 6 SECURITY DEFINER functions — *ADR-020*
- [x] 🔴 Resolve `is_primary` source-of-truth confusion (column lives on `user_profiles`) — *ADR-021*
...
- [~] **F9** Header navigation menu (Lectures / Study Plan / Progress) — stub nav links added (Slice 1); full active-state + mobile drawer in Slice 7
